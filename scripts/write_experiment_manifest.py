...
from export_v2xseq_official_tracking import OFFICIAL_RANGE
...
def main():
    ...
    parser.add_argument("--output", default="outputs/experiment_manifest.json")
    args = parser.parse_args()
    root = Path(__file__).resolve().parents[1]
    grae_config = load_config("configs/grae_centerpoint.yaml")
    classic_config = load_config("configs/ab3dmot_centerpoint.yaml")
    ...
    birth_path = root / grae_config["tracker"]["birth_thresholds_file"]
    birth_thresholds = yaml.safe_load(birth_path.read_text(encoding="utf-8"))["score_thresholds"]
    dair_commit = (root / "third_party" / "DAIR-V2X" / "COMMIT").read_text(encoding="utf-8").strip()
    ...
    payload = {
        ...
        "dair_v2x_commit": dair_commit,
        "val_sequences": len(val_ids),
        "official_extended_range": list(OFFICIAL_RANGE),
        "official_iou_threshold": 0.25,
        "custom_iou_thresholds": {
            str(name): float(value) for name, value in classic_config["evaluation"]["iou_thresholds"].items()
        },
